...
class CORSResource(SimpleTestResource):
    def on_options(self, req, resp, **kwargs):
        self.captured_req, self.captured_resp, self.captured_kwargs = req, resp, kwargs


class TestCors(testing.TestCase):
    def get_header(self, name):
        if self.resource.captured_resp is None:
            return None
    ...
    def simulate_cors_request(self, cors, route='/', preflight=False,
        ...
        if preflight:
            ...
            if add_request_method:
                kwargs['headers'].update({'access-control-request-method': preflight_method})
            method = 'OPTIONS'
        else:
        ...
        return getattr(self, 'simulate_{}'.format(method))(route, **kwargs)
